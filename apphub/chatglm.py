...
"""
    ...
    @property
    def install_env(self, install_location):
        ...
        if not self.in_work:
            self.execute_command(
                "conda create -y -n webui python==3.10.10",
            )
        else:
            self.execute_command(
                "conda create -y -n webui python==3.10.10",
    ...
    @property
    def env_name(self):
        return "base" if not self.in_work else "/home/featurize/work/app/chatglm/env"
    ...
    def installation(self, install_location):
        ...
        if  self.in_work:
            self.execute_command(
                "conda create -y --prefix /home/featurize/work/app/chatglm/env python=3.10.10"
            )

        source_path = os.path.join(install_location, "ChatGLM2-6B")
        with self.conda_activate(self.env_name):
            ...
            self.execute_command(
                "pip install --upgrade tabulate requests"
                )
            ...
            cwd = os.path.join(source_path, "THUDM")
            )   
    ...
    def start(self):
        ...
        with self.conda_activate(self.env_name):
            ...
            self.execute_command(
               f"streamlit run web_demo2.py --server.port {self.port}",
                cwd = os.path.join(self.cfg.install_location, "ChatGLM2-6B"),
                daemon = True
                )
            #result = subprocess.run('streamlit run web_demo2.py',shell = True, capture_output = True,
            #   cwd = os.path.join(self.cfg.install_location, "ChatGLM2-6B"))
